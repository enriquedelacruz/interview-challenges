...
public class CoffeeShopTest {
    ...
    public void testTakeMyFirstOrder(){
        ...
        coffeeShop.takeOrder(ProductType.LATTE, 1);
        coffeeShop.takeOrder(ProductType.ESPRESSO, 1);
        coffeeShop.takeOrder(ProductType.SANDWICH, 1);
        ...
        coffeeShop.takeOrder(ProductType.SANDWICH, 2);
        total = coffeeShop.printReceipt();
    ...
    public void testTakeMySecondOrder(){
        CoffeeShop coffeeShop = new CoffeeShop();
        coffeeShop.printMenu();
        coffeeShop.takeOrder(ProductType.LATTE, 2);
        coffeeShop.takeOrder(ProductType.ESPRESSO, 1);
        coffeeShop.takeOrder(ProductType.SANDWICH, 1);
        Double total = coffeeShop.printReceipt();
    ...
    public void testTakeMyFourthOrder(){
        ...
        coffeeShop.takeOrder(ProductType.LATTE, 1); //Now we have 3 lattes and we expect 1 free espresso
        total = coffeeShop.printReceipt();
        // Total should be $30.0 ( 5.3 + 5.3 + 4 + 4 + 10.10 + 5.3 - 4 [promotion for 2 lattes] = 30.0 )
        assertThat(total, is(30.0));

        coffeeShop.takeOrder(ProductType.LATTE, 1); //Now we have 3 lattes and we expect 2 free espressos
        total = coffeeShop.printReceipt();
        // Total should be $31.3 ( 5.3 + 5.3 + 4 + 4 + 10.10 + 5.3 + 5.3 - 8 [promotion for 4 lattes] = 31.3 )
        assertThat(total, is(31.3));

        coffeeShop.takeOrder(ProductType.ESPRESSO, 1); //Now we have 1 espresso more, but this won't be free
        total = coffeeShop.printReceipt();
